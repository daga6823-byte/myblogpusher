...
public class ExportController {
	...
	public ResponseEntity<byte[]> exportAll() throws Exception {
		...
		writeCsv(zos, "article_work.csv",
				new String[] { "work_id", "user_id", "category_group_id", "title", "content", "create_date",
						"update_date", "create_user", "update_user" },
				articleWorkRepository.findAll(),
				(ArticleWork w) -> new String[] {
						str(w.getWorkId()), str(w.getUserId()), str(w.getCategoryGroupId()), w.getTitle(),
						w.getContent(),
						str(w.getCreateDate()), str(w.getUpdateDate()), str(w.getCreateUser()), str(w.getUpdateUser())
				});

		writeCsv(zos, "typo_correction.csv",
				new String[] { "typo_id", "category_id", "wrong_word", "correct_word", "create_date", "update_date",
		...
		writeCsv(zos, "menu_master.csv",
				new String[] { "menu_url", "menu_name", "min_role", "create_date", "update_date", "create_user",
						"update_user" },
